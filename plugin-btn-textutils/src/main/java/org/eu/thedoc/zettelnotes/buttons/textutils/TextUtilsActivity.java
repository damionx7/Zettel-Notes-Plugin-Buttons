...
public class TextUtilsActivity extends AppCompatActivity {
  ...
  protected void onCreate (@Nullable Bundle savedInstanceState) {
    ...
    String[] formatOptions =
        {"Remove newlines", "Split to newlines", "Uppercase", "Lowercase", "Title Case", "Swap Case", "Create Task list", "Remove Task list", "Add Month Table",
         "Add Month Table With Empty Rows", "Add Year Table", "Add Year Table with Empty Rows"};
        ...
        switch (which) {
          case 8:
            setResult(RESULT_OK, new Intent()
                .putExtra(INTENT_EXTRA_INSERT_TEXT, CalendarUtils.getMonthTable(Calendar.getInstance().get(Calendar.YEAR), Calendar.getInstance().get(Calendar.MONTH), false)));
            break;
          case 9:
            setResult(RESULT_OK, new Intent()
                .putExtra(INTENT_EXTRA_INSERT_TEXT, CalendarUtils.getMonthTable(Calendar.getInstance().get(Calendar.YEAR), Calendar.getInstance().get(Calendar.MONTH), true)));
            break;
          case 10:
            setResult(RESULT_OK, new Intent().putExtra(INTENT_EXTRA_INSERT_TEXT, CalendarUtils.getYearTable(Calendar.getInstance().get(Calendar.YEAR), false)));
            break;
          case 11:
            setResult(RESULT_OK, new Intent().putExtra(INTENT_EXTRA_INSERT_TEXT, CalendarUtils.getYearTable(Calendar.getInstance().get(Calendar.YEAR), true)));
            break;
          default:
